...
int VectorMap::load_areas(const char *name)
{
	FILE *fp;
	char dummy[1000];
	Area a;
...
void VectorMap::load_poles (const std::string &polefilename)
{
	FILE *fp = fopen (polefilename.c_str(), "r");
	char dummy[1000];
	Pole p;
	fscanf(fp,"%s",dummy);
	while(fscanf(fp,"%d,%d,%lf ,%lf",
		&p.id, &p.vid, &p.length, &p.dim) != EOF) {
		poles[p.id] = p;
	}
	fclose (fp);
}
...
void VectorMap::loadAll (const std::string &_dirname)
{
	if (loaded)
		return;
	dirname = _dirname;
	...
	load_areas (areaname.c_str());

	load_poles (dirname + "/pole.csv");

	loaded = true;
}


vector<Point3> areaToPoints (Area &a, VectorMap &map)
{
	vector<Point3> pts;
	int fstpt = map.lines[a.slid].bpid;
	pts.push_back(map.getPoint(fstpt));

	for (int i=a.slid; i<=a.elid; i++) {
		Line li = map.lines[i];
		pts.push_back(map.getPoint(li.fpid));
	}
