/*
 * SignalsDrawObject.cpp
 *
 *  Created on: Apr 13, 2015
 *      Author: sujiwo
 */

#include "SignalsDrawObject.h"
#include "SceneManager.h"


SignalsDrawObject::SignalsDrawObject (VectorMap &v, bool doGL) :
	vmap (v),
	vbo (0)
{
	int ipts = 0;

	// check if maximum limit of this loop is right
	for (int i=1; i<vmap.signals.size()-1; i++) {
		Signal signal = vmap.signals[i];
		int pid = vmap.vectors[signal.vid].pid;
		Point3 pxyz = vmap.getPoint(pid);
		signalCenterPoints.push_back (pxyz);

		SignalCircle scirc;
		scirc.id = signal.id;
		switch (signal.type) {
		case 1: scirc.color = SIGNAL_RED; break;
		case 2: scirc.color = SIGNAL_GREEN; break;
		case 3: scirc.color = SIGNAL_YELLOW; break;
		}
		scirc.points = signalPoint2Circle (vmap, signal);
		scirc.points.push_back (pxyz);
		for (int j=0; j<scirc.points.size(); j++) {
			scirc.idx.push_back(ipts);
			ipts++;
		}
		allPts.insert(allPts.end(), scirc.points.begin(), scirc.points.end());
		signalObjects.push_back(scirc);
	}

	if (doGL)
		glGenBuffers (1, &vbo);
}


SignalsDrawObject::~SignalsDrawObject ()
{
	if (vbo !=0)
		glDeleteBuffers (1, &vbo);
}


void SignalsDrawObject::initialize ()
{
	glBindBuffer (GL_ARRAY_BUFFER, vbo);

	glBufferData (GL_ARRAY_BUFFER,
		sizeof(Point3)*allPts.size(),
		allPts.data(),
		GL_STATIC_DRAW);

	glBindBuffer (GL_ARRAY_BUFFER, 0);
}


void SignalsDrawObject::draw()
{
	scene->getShader()->use();
	glBindBuffer (GL_ARRAY_BUFFER, vbo);
	glEnableClientState (GL_VERTEX_ARRAY);
	glVertexPointer (3, GL_FLOAT, 0, 0);

//	glPointSize (1.0);
//	glDrawArrays (GL_POINTS, 0, signalCenterPoints.size());
	for (int i=0; i<signalObjects.size(); i++) {
		SignalCircle &signal = signalObjects[i];
		switch (signal.color) {
		case SIGNAL_RED:
			setColor(255, 0, 0); break;
		case SIGNAL_GREEN:
			setColor(0, 255, 0); break;
		case SIGNAL_YELLOW:
			setColor(255, 255, 0); break;
		}
		doSetColor ();
		glDrawElements (GL_TRIANGLE_FAN, signal.idx.size(), GL_UNSIGNED_INT, signal.idx.data());
	}

	//glFlush ();

	glDisableClientState (GL_VERTEX_ARRAY);
	glBindBuffer (GL_ARRAY_BUFFER, 0);
}


Vector3 SignalsDrawObject::angle2UnitVector (const float Hang, const float Vang)
{
	Vector3 unv;
	unv.x() = sin (Hang * M_PI/180.0);
	unv.y() = cos (Hang * M_PI/180.0);
	unv.z() = 0.0;
	unv.normalize();
	return unv;
}


vector<Point3> SignalsDrawObject::signalPoint2Circle (const Point3 &c, float Hang, float Vang)
{
	vector<Point3> circle;

	for (double angle=-M_PI; angle<M_PI; angle+=M_PI/5.0) {

		Point3 p;
		double u=0.3*cos(angle);
		double v=0.3*sin(angle);
		p.x() = c.x() + u*cos(Hang*M_PI/180.0);
		p.y() = c.y() - u*sin(Hang*M_PI/180.0);
		p.z() = c.z() + v;
		circle.push_back (p);

		u=0.3*cos(angle+M_PI/5);
		v=0.3*sin(angle+M_PI/5);
		p.x() = c.x() + u*cos(Hang*M_PI/180.0);
		p.y() = c.y() - u*sin(Hang*M_PI/180.0);
		p.z() = c.z() + v;
		circle.push_back (p);
	}

	return circle;
}


vector<Point3> SignalsDrawObject::signalPoint2Circle (VectorMap &map, const Signal &sgn)
{
	Vector v = map.vectors[sgn.vid];
	int pid = v.pid;
	Point3 pc = map.getPoint(pid);
	return signalPoint2Circle (pc, v.hang, v.vang);
}


Point3 multiply3to4 (const Matrix4 &mat, const Point3 &point, float w)
{
	Point4 pw (point.x(), point.y(), point.z(), w);
	Point4 r4 = mat * pw;
	return Point3 (r4.x(), r4.y(), r4.z());
}


Point4 p3to4 (const Point3 &p3)
{
	return Point4 (p3.x(), p3.y(), p3.z(), 1);
}


Point3 p4to3 (const Point4 &p4)
{
	return Point3 (p4.x(), p4.y(), p4.z());
}


/* XXX: Subject to change */
bool SignalCircle::project (Camera &camera, int &u, int &v, float &projectedRadius)
{
	//return camera.project(center, u, v);
	Point4 center4 = p3to4 (center),
		// shadow point for finding projected radius
		center4r = center4;
	center4r.z() += lampRadius;
	center4 = camera.getViewMatrix() * center4;
	center4r = camera.getViewMatrix() * center4r;

	double far = 100.0;

	// filter point
	if (center4.z() > 0 or abs(center4.z()) > far)
		return false;
	Point4 pcam = camera.getProjectionMatrix() * center4;
	Point4 pcamr = camera.getProjectionMatrix() * center4r;
	pcam = pcam / pcam.w();
	pcamr = pcamr / pcamr.w();
	if (pcam.x()<-1 or pcam.x()>1 or pcam.y()<-1 or pcam.y()>1)
		return false;

	projectedRadius = distance (pcamr, pcam) * camera.getHeight();

	pcam.x() = (pcam.x() + 1.0) / 2;
	pcam.y() = (pcam.y() + 1.0) / 2;

	float up = (float)camera.getWidth() * pcam.x(),
		vp = (float)camera.getHeight() * pcam.y();

	u = (int)up, v = (int)vp;


	return true;
}


Point3 SignalCircle::transform (Camera &camera)
{
	return camera.transform(center);
}
